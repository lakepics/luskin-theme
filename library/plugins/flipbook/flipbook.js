...
(function () {
    ...
    var pages = [],
        ...
        /* slide show delay in miliseconds */
        slideshowTimeout;
    ...
    function setHashTag(flipbook) {
        ...
        if (pages[currentID] != "" && pages[currentID] != undefined) {
            ...
            if (currentID % 2 != 0) pageURL = getURL() + '#' + currentID;
            else if (currentID == flipbook.data().totalPages) pageURL = getURL() + '#' + currentID;
            else pageURL = getURL() + '#' + (currentID + 1);
        }
    ...
    function hideShadows(caller, arrow, flipbook, fbCont, posLeft) {
        ...
        if (caller == 'start' || animate) active = flipbook.activePageCorner();

        if (caller == 'start' && active != 'tl' && active != 'bl' && active != 'tr' && active != 'br' || animate && active != 'tl' && active != 'bl' && active != 'tr' && active != 'br') {
            active = arrow;
        }
        ...
        if (page == 2 && animate && (active == 'tl' || active == 'bl' || active == 'left')) {
            flipbook.children('div.fb-shadow-top-left').animate({
                opacity: 0
            }, 200);
            flipbook.children('div.fb-shadow-bottom-left').animate({
                opacity: 0
            }, 200);
        } else if (page == flipbook.data().totalPages - 2 && animate && (active == 'tr' || active == 'br' || active == 'right')) {
            flipbook.children('div.fb-shadow-top-right').animate({
                opacity: 0
            }, 200);
            flipbook.children('div.fb-shadow-bottom-right').animate({
                opacity: 0
            }, 200);
        }
    ...
    function resizeFB(fbWidth, fbHeight, flipbook, fbCont, zoomed) {
        ...
        if (zoomed) {
            ...
            if (zoomCont.find('img.bg-img').hasClass('zoom-large')) largeImage = true;
        ...
        flipbook.find('div.page-content').each(function () {
            ...
            if ($this.find('object, iframe').length) $this.find('.preview-content').height('100%');
    ...
    function centerBook(page, flipbook, fbCont, activeArrow) {
        ...
        if (page == 1 || page == flipbook.data().totalPages) {
            ...
            if (page == 1) left = Math.floor((fbCont.width() - pageWidth) * 0.5) - pageWidth;
            else left = Math.floor((fbCont.width() - pageWidth) * 0.5);
    ...
    function fbFirstRun(flipbook, fbCont) {
        ...
        if (jQuery('div.fb-nav').hasClass('small')) {
            jQuery('div.fb-nav').animate({
                opacity: 1,
                top: -20 - parseInt(flipbook.css('margin-bottom').replace("px", "")) - parseInt(jQuery('div.fb-nav').css('margin-top').replace("px", ""))
            }, 1000, 'easeOutExpo');
        } else {
            jQuery('div.fb-nav').animate({
                opacity: 1,
    ...
    jQuery(document).ready(function ($) {
        ...
        flipbook.bind('turned', function (e, page) {
            ...
            if (slideshow) {
                slideshowTimeout = setTimeout(function () {
                    ...
                    if (flipbook.turn('page') + 2 >= flipbook.data().totalPages) {
                        // turn off slide show on last slide

                        slideshow = false;
                        fbNav.find('ul li.slideshow span.button-icon-over').animate({
                            opacity: 0
                        }, 300);
                        fbNav.find('ul li.slideshow span.button-icon').animate({
                            opacity: 1
                        }, 300);
        ...
        flipbook.find('div.turn-page-wrapper').each(function () {
            ...
            if (pageID % 2 != 0 && pageID != 1 && pageID != lastID) {
                var rightMargin = parseInt($this.find('div.double div.page-content').css('margin-right'));
        ...
        flipbook.find(' > div:last-child > div').each(function () {
            ...
            if (pageID == 1) $this.addClass('first');
            else if (pageID == lastID) $this.addClass('last');
            else if (pageID % 2 == 0) $this.addClass('even');
        ...
        flipbook.find('div.turn-page-wrapper').dblclick(function (e) {
            ...
            if (slideshow) {
                ...
                fbNav.find('ul li.slideshow').children('span.button-icon-over').css({
                    opacity: 0
                });
                fbNav.find('ul li.slideshow').children('span.button-icon').css({
                    opacity: 1
                });
                fbNav.find('ul li.slideshow').children('span.button-over').css({
                    opacity: 0
                });
                fbNav.find('ul li.slideshow').children('span.button').css({
                    opacity: 1
                });
            }

            flipbook.parent().prepend($(this).find('div.page-content').clone(true).addClass('zoomed')).css('opacity', 0).animate({
                opacity: 1
            }, 500);

            flipbook.parent().prepend('<div class="zoomed-shadow"></div>').css('opacity', 0).animate({
                opacity: 1
            }, 500);
            ...
            if ($page.is('.first, .last')) {
                if ($page.hasClass('first')) $prev.stop(true, true).fadeOut(100);
                else $next.stop(true, true).fadeOut(100);
            }
            ...
            if (e.pageY == undefined) e.pageY = fbTopMargin;
            ...
            $prev.css({
                'left': l - 40
            }).height(h).children('.center, .center.hover').height(h - 24);
            ...
            function closeZoom() {
                ...
                fbCont.find('div.zoomed-shadow').animate({
                    opacity: 0
                }, 300, function () {
                    $(this).remove();
                });
                fbCont.find('div.zoomed-shadow-bottom').animate({
                    opacity: 0
                }, 100, function () {
                    $(this).remove();
                });
                fbCont.find('div.zoomed-shadow-top').animate({
                    opacity: 0
                }, 100, function () {
                    $(this).remove();
                });
            ...
            function reinitZoom() {
                ...
                fbCont.find('div.page-content.zoomed').animate({
                    opacity: 0
                }, 100, function () {
                    $(this).remove();
                });
            ...
            fbCont.find('div.zoomed-shadow-top, div.zoomed-shadow-bottom').width(zoomCont.width()).animate({
                opacity: 1
            }, 500);
            fbCont.find('div.zoomed-shadow-top').css({
                top: position.top - 13,
        ...
        $('.fb-nav ul li, .flipbook-container div.next, .flipbook-container div.preview').hover(function () {
            ...
            $this.children('span.button').animate({
                opacity: 0
            }, 300);
            $this.children('span.button-over').animate({
                opacity: 1
            }, 300);
        }, function () {
            var $this = $(this);
        ...
        fbNav.find('ul li.toc').on('click', function (e) {
            if (flipbook.turn('page') != tocIndex) flipbook.trigger('mouseover');
        ...
        fbNav.find('ul li.zoom').on('click', function (e) {
            flipbook.find('div.turn-page-wrapper').each(function () {
                var $this = $(this);
                if ($this.attr('page') == flipbook.turn('page') && !zoomed) $this.trigger('dblclick');
                else if ($this.attr('page') == flipbook.turn('page') && zoomed) fbCont.find('div.zoomed').trigger('dblclick');
            });
        });
        ...
        fbNav.find('ul li.slideshow').on('click', function (e) {
            ...
            if (slideshow) {
                ...
                $this.children('span.button-icon-over').animate({
                    opacity: 1
                }, 300);
                $this.children('span.button-icon').animate({
                    opacity: 0
                }, 300);
            } else {
                clearTimeout(slideshowTimeout);
        ...
        fbNav.find('ul li.show-all').on('click', function (e) {
            ...
            fbCont.find('div.preview, div.next').animate({
                opacity: 0
            }, 300);
            ...
            flipbook.find('div.page-content').each(function () {
                ...
                if (!$this.parent().parent().hasClass('fpage')) {
                    ...
                    if ($this.parent().parent().parent().hasClass('last')) clone.addClass('even');
                    else if ($this.parent().parent().parent().hasClass('first')) clone.addClass('odd');

                    // if page double add class double
                    if ($this.parent().hasClass('double')) clone.addClass('double');
                    // append clone to show-all div
                    if ($this.parent().parent().parent().hasClass('last')) showAll.find('div.content').prepend(clone);
                    else showAll.find('div.content').append(clone);
            ...
            if (showAll.height() < showAll.find('div.content').height() - paddingVertical) {
                ...
                fbCont.find('span.show-all-previous').animate({
                    opacity: 1
                }, 500);
                fbCont.find('span.show-all-next').animate({
                    opacity: 1
                }, 500);
            }
            ...
            showAll.find('div.show-all-thumb').hover(function () {
                ...
                $this.find('span.shadow').animate({
                    opacity: 1
                }, 300);

                if ($this.hasClass('even')) {
                    $this.next().find('span.shadow').clearQueue();
                    $this.next().find('span.shadow').animate({
                        opacity: 1
                    }, 300);
                } else {
                    $this.prev().find('span.shadow').clearQueue();
                    $this.prev().find('span.shadow').animate({
                        opacity: 1
                    }, 300);
                }
            }, function () {
            ...
            fbCont.find('div.showall-shadow-top, div.showall-shadow-bottom').width(showAll.width()).delay(500).animate({
                opacity: 1
            }, 500);
            ...
            showAll.find('div.show-all-thumb').on('click', function () {
                ...
                fbCont.find('div.showall-shadow-bottom').animate({
                    opacity: 0
                }, 100, function () {
                    $(this).remove();
                });

                fbCont.find('div.showall-shadow-top').animate({
                    opacity: 0
                }, 100, function () {
                    $(this).remove();
                });
            ...
            fbCont.find('span.show-all-previous').on('click', function () {
                if (parseInt(saContent.css('top')) != paddingAround && !animation) {
                    ...
                    saContent.animate({
                        top: parseInt(saContent.css('top')) + scrollAmount
            ...
            fbCont.find('span.show-all-next, span.show-all-previous').hover(function () {
                var but = $(this);
                but.clearQueue();
                but.find('span.left.hover, span.right.hover, span.center.hover').animate({
                    opacity: 1
                }, 300);
            }, function () {
                var but = $(this);
        ...
        firstWidth = flipbook.width(), firstHeight = flipbook.height();
        ...
        $(window).on('resize', function () {
            ...
            if ($this.height() - fbNav.offset().top + 150 < 0) height = $this.height() - flipbook.offset().top - ($this.height() - fbNav.offset().top + 150);
            else height = $this.height() - flipbook.offset().top - 20;
            ...
            if (fbCont.height() / firstHeight < fbCont.width() / firstWidth) ratio = fbCont.height() / firstHeight;
            else ratio = fbCont.width() / firstWidth;
            ...
            if (fbWidth != flipbook.width() || fbHeight != flipbook.height()) resizeFB(fbWidth, fbHeight, flipbook, fbCont, zoomed);
            ...
            if (currentID == 1 || currentID == flipbook.data().totalPages) {
                ...
                if (currentID == 1) left = Math.floor((fbCont.width() - flipbook.width() / 2) * 0.5) - flipbook.width() / 2;
                else left = Math.floor((fbCont.width() - flipbook.width() / 2) * 0.5);

                if (parseInt(flipbook.css('left')) != left) flipbook.css({
                    'left': left
                });
            } else {
                flipbook.css({
                ...
                if (zoomed) {
                    fbCont.children('div.zoomed').css({
                        'left': Math.floor((fbCont.width() - flipbook.width()) * 0.5) - 10
                    });
                    fbCont.children('div.zoomed-shadow, div.zoomed-shadow-bottom, div.zoomed-shadow-top').css({
                        'left': Math.floor((fbCont.width() - flipbook.width()) * 0.5)
                    });
                }
            }
            ...
            /* Preview / Next Arrows */
            if (width < areaWidth) {
                fbNext.addClass('small').css({
                    left: position.left + flipbook.width() - 40
                });
                fbPrev.addClass('small').css({
                    left: position.left - 45
                });
                fbPrev.find('span.button-icon').css({
                    left: 6
                });
                fbNext.find('span.button-icon').css({
                    left: 41
                });
            } else if (width > areaWidth) {
                fbNext.removeClass('small').css({
                    left: position.left + flipbook.width() + 24
                });
                fbPrev.removeClass('small').css({
            ...
            if (fbPrev.css('opacity') == "0") fbPrev.css('left', '100px');

            if (fbNext.css('opacity') == "0") fbNext.css('right', '100px');

            if (zoomed) {
                var l = parseInt(fbCont.find('div.zoomed').css('left'));

                fbCont.children('.big-side.show-previous').css({
                    'left': l - 40
                }).height(fbHeight).children('.center, .center.hover').height(fbHeight - 24);

                fbCont.children('.big-side.show-next').css({
                    'right': l - 40
                }).height(fbHeight).children('.center, .center.hover').height(fbHeight - 24);
            }
            ...
            if (height < areaHeight) {
                fbNav.addClass('small').css({
                    top: -40 - parseInt(flipbook.css('margin-bottom').replace("px", "")) - parseInt(fbNav.css('margin-top').replace("px", ""))
                });
                fbNav.find('ul > li.toc > span.button-icon').css({
                    top: 40
                });
                fbNav.find('ul > li.zoom > span.button-icon').css({
                    top: 38
                });
                fbNav.find('ul > li.slideshow > span.button-icon, ul > li.slideshow > span.button-icon-over').css({
                    top: 39
                });
                fbNav.find('ul > li.show-all > span.button-icon').css({
                    top: 40
                });
            } else if (height > areaHeight) {
                fbNav.removeClass('small').css({
                    top: 0
                });
                fbNav.find('ul > li.toc > span.button-icon').css({
                    top: 25
        ...
        $(window).bind('keydown', function (e) {
            ...
            if (e.keyCode == 37) {
                ...
                fbCont.find('div.zoomed').animate({
                    opacity: 0
                }, 300, function () {
                    $(this).remove();
                    zoomed = false;
            ...
            // hashchange event (unique url for each page)
            hashChange(flipbook, fbCont);
        }).bind('touchstart', function (e) {
            // touch events for mobile stuff
            ...
            if (ie) {
                flipbook.find('div.fpage object').css({
                    'display': 'none'
                });
                flipbook.find('div.video object').css({
                    'display': 'none'
                });
            } else {
                flipbook.find('div.fpage object').css({
