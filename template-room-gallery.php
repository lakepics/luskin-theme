<?php
/*
 * Template Name: Room Image Gallery
 */
?>
<?php get_header(); ?>
...
<div id="main-content" role="main">
	<?php if (have_posts()) : ?>
		...
		<!-- <?php
			...
			foreach (get_field('room-images', 457) as $picture){
				$categories[create_slug( $picture['room-gallery-category'])] =  $picture['room-gallery-category'];
			}
		?> 
		...
		<?php
			...
			foreach ($pictures as $picture){
				...
				if( $picture['large-room-image'] ){
					...
					echo '<a href="'. $picture['large-room-image']['url'] .'" title="'. $imgTitle .'" rel="masonry-gallery">';
